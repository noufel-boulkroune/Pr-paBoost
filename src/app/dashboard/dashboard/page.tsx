...
import { useEnrollments, useStudentStats } from "@/features/users/hooks/useUser";
    ...
    <div className="space-y-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {statCards.map((stat) => (
            <Card key={stat.label} className="relative overflow-hidden">
              {/* Left accent */}
              <div className={`absolute left-0 top-0 bottom-0 w-1 bg-gradient-to-b ${stat.gradient} rounded-l-2xl`} />
              <CardContent className="flex items-center gap-4 pl-5">
                <div className={`w-12 h-12 rounded-xl bg-gradient-to-br ${stat.gradient} text-xl flex items-center justify-center shadow-sm flex-shrink-0`}>
                  {stat.icon}
                </div>
                <div className="flex-1 min-w-0">
                  ...
                  {stat.trend && (
                    <p className={`text-caption mt-1 font-medium ${stat.trendUp ? "text-success-600" : "text-text-muted"}`}>
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        <Card className="lg:col-span-1">
          <CardContent>
            <h2 className="text-heading-lg font-bold text-text-primary mb-1">Weekly Activity</h2>
            <p className="text-caption text-text-muted mb-4">Minutes learned per day</p>
            <div className="flex items-end justify-between gap-1.5 h-28">
              {activityData.map((val, i) => (
                <div key={i} className="flex-1 flex flex-col items-center gap-1">
                  ...
                  <span className="text-caption text-text-muted">{activityLabels[i]}</span>
                </div>
              ))}
            </div>
          </CardContent>
          ...
          {enrollmentsLoading ? (
                  ...
                  <CardContent className="flex flex-col sm:flex-row sm:items-center gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="mt-2">
                        ...
                        <ProgressBar value={enrollment.progress.progressPercentage} size="sm" />
                      </div>
                    </div>
            ...
            <Card variant="gradient">
              <CardContent className="text-center py-10">
                ...
                <h3 className="text-heading-md font-bold text-text-primary mb-2">No courses yet</h3>
        ...
        <Card variant="gradient" className="border-dashed border-primary-200">
          <CardContent className="text-center py-10">
            ...
            <p className="text-body-md text-text-secondary">
              Complete a few courses and we&apos;ll tailor recommendations just for you.
            </p>
          </CardContent>
        </Card>
      </div>
